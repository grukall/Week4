...
struct FNameComparisonValue : public FNameValue
{
		...
		: FNameValue(InName)
	{
		...
		size_t Len = min(InName.length(), size_t(NAME_SIZE - 1));
...
private:
	...
	{
		ComparisonHashBuckets.Init(FNameSlot(), static_cast<unsigned int>(InitialCapacity));
		DisplayHashBuckets.Init(FNameSlot(), static_cast<unsigned int>(InitialCapacity));
	}
...
FString FName::ToString() const
{
	...
	{
		Result.AppendChar('-');
		Result.Append(FString(std::to_string(Number - 1)));
	}
